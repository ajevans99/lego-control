...
class AllLightsBrightnessMutation(graphene.Mutation):
    class Arguments:
        brightness = graphene.Int()

    # The class attributes define the response of the mutation
    brightness = graphene.Int()
    light_strips = graphene.List(LightStripNode)

    @classmethod
    def mutate(cls, root, info, brightness):
        if brightness > 100 or brightness < 0:
            return ValueError(f'Brightness value should be between 0 and 100')
        light_strips = LightStrip.objects.all()
        for light_strip in light_strips:
            light_strip.brightness = brightness
            light_strip.save()
        # Notice we return an instance of this mutation
        return AllLightsBrightnessMutation(light_strips=LightStrip.objects.all(), brightness=brightness)
...
class Mutation(graphene.ObjectType):
    all_lights_brightness_mutation = AllLightsBrightnessMutation.Field()
...
class LightStripBrightnessMonitor(channels_graphql_ws.Subscription):
    ...
    @staticmethod
    def subscribe(root, info, control_point_id):
        """Called when user subscribes."""
        
        print(f"SUBSCRIBED - {control_point_id}")

        return [str(control_point_id)]
    ...
    def publish(payload, info, control_point_id):
        ...
        return LightStripBrightnessMonitor(
            ...
            light_strip_id=payload['light_strip_id'],
